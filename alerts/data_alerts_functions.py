...
DEPLOYMENT_URL = 'https://ooinet.oceanobservatories.org/api/m2m/12587/events/deployment/inv/'
DATA_URL = 'https://ooinet.oceanobservatories.org/api/m2m/12576/sensor/inv/'
DATA_TEAM_PORTAL_URL = 'http://ooi.visualocean.net/data-streams/science/'

out_dir = '/home/cwingard/data/ooi_stats/alerts/output/'
...
def request_data(url, api_key, api_token):
...
def request_gr(api_key, api_token):
    print("requesting qc data...")
    r = requests.get(QC_PARAMETER_URL, auth=(api_key, api_token))
    data = r.json()
...
def request_annotations(array, api_key, api_token):
    beginDT = int(nc.date2num(datetime.datetime.strptime("2012-01-01T01:00:01Z", '%Y-%m-%dT%H:%M:%SZ'),
                              'seconds since 1970-01-01') * 1000)
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=20) as pool:
        ...
        future_to_url = {pool.submit(request_data, url, api_key, api_token): url for url in anno_requests}
    ...
    data_dict = {'refdes': ref_des_list}
    not_operational = pd.DataFrame(data_dict, columns=['refdes'])
    return not_operational


def alert_request_deployments(array, api_key, api_token):
    refdes_in = DATA_TEAM_PORTAL_URL + array
    refdes_list = pd.read_csv(refdes_in)
    refdes_list = refdes_list[['reference_designator', 'method', 'stream_name', 'parameter_name']]
    refdes_list.columns = ['refdes', 'method', 'stream', 'parameter']
    ...
    for i in refdes_list:
        ...
        request_url = DEPLOYMENT_URL+asset_url_inputs+'/-1'
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=20) as pool:
        future_to_url = {pool.submit(request_data, url, api_key, api_token): url for url in asset_requests}
        for future in concurrent.futures.as_completed(future_to_url):
            try:
...
def send_gr_data_requests(request_urls, global_ranges, api_key, api_token):
    print('    sending data requests...')
    ooi_parameter_data_gr = pd.DataFrame()
    missing = []

    with concurrent.futures.ThreadPoolExecutor(max_workers=20) as pool:
        future_to_url = {pool.submit(request_data, url, api_key, api_token): url for url in request_urls}
        for future in concurrent.futures.as_completed(future_to_url):
            ...
            try:
                    ...
                    if y.empty:
                        ...
                        for var in y.parameter.values:
                            for j in data[i].keys():
                                if var == j:
                                    z = data[i][j]
                                    # conditional to handle 2d data sets, in which case the first
                                    # non NaN value is checked
                                    if type(z) != list:
                                        refdes_list.append(refdes)
                                        method_list.append(method)
                                        stream_list.append(stream)
                ...
                data_dict = {
                    ...
                    'value': value_list,
                    'date': timestamp_list}
                response_data = pd.DataFrame(data_dict, columns=['refdes', 'method', 'stream',
    ...
    # subset response to only return instances where more than 50% of the data points returned pass the global range
    # test. Time always passes the test, so stream availability is not lost.
    try:
...
def alert_create_all_outputs(ooi_parameter_data_gr, request_inputs):
...
def alert_create_missing_output(array, param_final, stream_final, method_final, refdes_final, missing_gr_qc_values):
    ...
    # only capture parameters that are actually failing the global range test, not missing because no values
    # have been entered.
...
def parameter_compare_output(param_final_out, param_most_recent, request_inputs):
    try:
...
def send_email(subject, html, recipients):
    ...
    # load the required authentication information
    secrets = netrc.netrc()
    user, _, password = secrets.authenticators('ooice.platforms@gmail.com')

    # compose the message with appropriate headers
    ...
    # compile the message components and send the message
    try:
...
def alert_send(array, no_data_not_annotated, annotated_and_not_operational, data_but_annotated, stream_difference_new,
               stream_difference_resumed, param_final_out, param_difference_new, param_difference_resumed,
               missing_gr_qc_values, missing_science_classification, ca_recipients, ea_recipients,
               cgsn_recipients):
    ...
    if stream_difference_new.empty and data_but_annotated.empty and stream_difference_resumed.empty and \
            param_difference_new.empty and param_difference_resumed.empty:
        text = '<h2>Ongoing Issues</h2>'
        pass
    else:
        text = '<h2><font color="red">New Alert</font></h2>'
        text = text + ('<br><small>*Alerts are only sent once when there is a new alert. If an instrument ' +
                       'resumes producing data a new alert will be sent.</small><br><br>')

    if stream_difference_new.empty: 
        pass
    else:
        text = text + ('<b>Instruments and streams that <font color="red">have not produced data</font> in the ' +
                       'past 24 hours:</b><br><br>')
    ...
    if data_but_annotated.empty:
        ...
        text = text + ('<b>Instruments that are <font color="red">annotated as not_operational, but have ' +
                       'resumed producing data:</font></b><br><br>')
    ...
    if param_difference_new.empty:
        ...
        text = text + ('<b>Parameters <font color="red">not producing data within global ranges</font> ' +
                       'in the past 24 hours:</b><br><br>')
    ...
    if stream_difference_resumed.empty:
        ...
        text = text + ('<b>Instruments and streams that <font color="green">resumed producing data</font> ' +
                       'in the past 24 hours:</b><br><br>')
    ...
    if param_difference_resumed.empty:
        ...
        text = text + ('<b>Parameters that <font color="green">resumed producing data within global ranges' +
                       '</font> in the past 24 hours:</b><br><br>')
    ...
    if missing_gr_qc_values.empty:
        ...
        text = text + ('<small>Values need to be entered here: https://github.com/ooi-integration/qc-lookup/' +
                       'blob/master/data_qc_global_range_values.csv</small><br><br>')
    ...
    if missing_science_classification.empty:
        ...
        text = text + ('Parameter instances with QC values but <b>not classified as science parameters:</b> ' +
                       'but that have global range qc values<br><br>')
        text = text + ('<small>Classification needs to be made in preload as "Science Data" under column ' +
                       'dataproducttype: https://github.com/oceanobservatories/preload-database/blob/'
                       'master/csv/ParameterDefs.csv</small><br><br>')
        text = text + ('<small>If the parameter does not exist in preload, then the parameter needs to be ' +
                       'removed from the QC tables https://github.com/ooi-integration/qc-lookup/blob/' +
                       'master/data_qc_global_range_values.csv</small><br><br>')
        f = missing_science_classification.to_html()
        text = text + str(f) + '<br><br>'
    subject = 'Data Quality and Availability Alert for ' + array + ' on ' + \
              datetime.datetime.utcnow().strftime('%Y-%m-%d')
    html = print_html_doc(html_template, text)

    if array == 'RS':
        send_email(subject, html, ca_recipients)
    elif array == 'CE':
        send_email(subject, html, ea_recipients)
    else:
        send_email(subject, html, cgsn_recipients)
