...
def request_data(url, api_key, api_token):
    return session.get(url, auth=(api_key, api_token))
...
def get_most_recent_eng(array):
    try:
        ...
        print('Creating empty recent engineering data streams')
...
def alert_request_eng_deployments(array, api_key, api_token):
    refdes_in = DATA_TEAM_PORTAL_URL + array
    refdes_list = pd.read_csv(refdes_in)
    refdes_list = refdes_list[refdes_list['method'].str.contains("recovered") == False]
    refdes_list = refdes_list[refdes_list['stream_type'].str.contains("Science") == False]
...
def send_eng_data_requests(array, request_urls, api_key, api_token):
...
def alert_create_eng_outputs(eng_streams_data, request_inputs):
    # stream level rollup
...
def alert_send(array,no_data_not_annotated, annotated_and_not_operational, data_but_annotated, stream_difference_new,
               stream_difference_resumed, ca_recipients, ea_recipients, cgsn_recipients):
    html_template = """
    <html>
    <body>{{ body }}</body>
    </html>
    ...
    if stream_difference_new.empty and data_but_annotated.empty and stream_difference_resumed.empty:
        ...
        text = text + ('<small>*Alerts are only sent once when there is a new alert. If an instrument resumes' +
                       'producing data a new alert will be sent.</small><br>')

    if stream_difference_new.empty: 
        pass
    else:
        text = text + ('<b>Engineering streams that <font color="red">have not produced data</font> in ' +
                       'the past 24 hours:</b><br>')
    ...
    if data_but_annotated.empty:
        ...
        text = text + ('<b>Instruments that are <font color="red">annotated as not_operational, but have ' +
                       'resumed producing data:</font></b><br>')
    ...
    if stream_difference_resumed.empty:
        ...
        text = text + ('<b>Engineering streams that <font color="green">resumed producing data</font> in the ' +
    ...
    if no_data_not_annotated.empty:
        ...
        text = text + 'Engineering streams <b>not producing data</b> for over 24 hours:<br>'
    ...
    if annotated_and_not_operational.empty:
        ...
        text = text + 'Engineering streams <b>not producing data and annotated as not_operational</b>:<br>'
